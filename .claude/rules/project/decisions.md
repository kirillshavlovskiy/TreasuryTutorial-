...
**Anti-patterns:** Do not check policy approval thresholds against sum of standalone VARs — that ignores diversification and inflates the number. Do not treat the correlation matrix as static forever — recalibrate when monetary policy regime changes (especially for EM currencies in rate-transition phases). Currencies not in CORR_CURRENCIES (AED, CNY, CZK, DKK, HKD, NZD, RON, RSD, SGD, THB) are excluded from portfolio VAR — they are included in per-currency buffer calculation but not in the portfolio panel. CAD and ILS were added to CORR_CURRENCIES after this decision was written and are now included (14 currencies: EUR, GBP, AUD, JPY, CHF, CAD, MXN, TRY, ZAR, PLN, NOK, SEK, HUF, ILS).
**Ticket:** —
...
## Project-local `fx-*` skill set, self-contained, reviews on a separate agent

**Date:** 2026-08-24
**Decision:** Agent skills for this repo live in `.claude/skills/` as six `fx-*` skills that depend on nothing outside this repository, and every review inside them runs in a **separate sub-agent on Opus 5 or better** with reasoning effort high or above.
**Alternatives considered:** Depend on the org-wide `deel-agent-tools` set (131 skills) — rejected: a non-engineer who clones only this repo would not have it, and the set assumes Deel product repos, SpiceDB, NATS and a multi-repo worktree layout that do not exist here. Reviewing inline in the same context that wrote the code — rejected, see Reason. Unprefixed names (`review`, `code-review`) — rejected: `treasury` already has `code-review` and `deel-agent-tools` has `codereview`, and all three can be additional working directories in one session.
**Reason:** The repo already held its rules in prose (`CLAUDE.md`, `.claude/rules/`), so nothing was enforced unless a person remembered to read a file. On the sub-agent requirement the evidence is from building this set: review round 1 on this very change found **3 CRITICAL** bugs the authoring context had missed (the hook blocked `/sync-dept`; `..` traversal walked past the guard; case-insensitive APFS made `dockerfile`/`HELM/` editable), and round 2 found 7 more — including a statistic the authoring context had fabricated from a sloppy grep. An agent is measurably bad at reviewing its own output.
**Anti-patterns:** Do not add a dependency on `deel-agent-tools` — port what is needed instead. Do not review a diff inline in the context that produced it. Do not run a review on a smaller model to save tokens. Do not claim in a report that reasoning effort was set to high when the `Agent` tool was used — `Agent` accepts `model` but has no `effort` parameter; effort comes from the session or from `Workflow`'s `agent()`. Do not name a skill something that collides with `treasury` or `deel-agent-tools`.
**Ticket:** GL-926

## Protected-paths hook: absolute for deployment, in-place-edits-only for knowledge layers

**Date:** 2026-08-24
**Decision:** `.claude/hooks/protected-paths.mjs` blocks `helm/`, `argocd/`, `.github/`, `Dockerfile` and `values.yaml` for every tool **the PreToolUse matcher covers** (Edit, Write, NotebookEdit, MultiEdit — Bash is not matched), but blocks `.claude/rules/{dept,div}/` only for in-place edits — a full `Write` is allowed. Exemption is an **allowlist** of full-overwrite tools (`write`), never a blocklist of editors.
**Alternatives considered:** Block every write to `dept/`/`div/` — rejected: `/sync-dept` and `/sync-division` refresh those layers *by overwriting those exact files*, so the guard would make the layers impossible to update ever again, and its own refusal message named the command it had just blocked. Drop the layers from the hook entirely — rejected, it removes the guard against the common accidental hand-tweak. Route the sync commands through Bash (unmatched by the hook) so `Write` could also be blocked — deferred: those files are generated by `claude-deel-treasury init` and a local edit may be overwritten by the CLI; that is a separate decision.
**Reason:** The hook exists because prose rules can be argued past and a hook cannot. But a guard that breaks the only legitimate write path is worse than none — it gets removed wholesale. Splitting by tool keeps the absolute guarantee where nothing legitimate writes (deployment) and a partial one where something does (knowledge layers). The remaining hole is real and is stated plainly in `.claude/skills/README.md`: a hand-written full overwrite passes the hook, and the rest of the defence is `fx-pre-push` step 2, which is model-following prose.
**Anti-patterns:** Do not describe this guard as absolute or as covering "every tool" — the matcher is file tools only, Bash reaches any path, and the hook plus `.claude/settings.json` are themselves ordinary editable files. Do not implement the exemption as a blocklist of in-place editors — any tool name not on that list silently inherits the exemption, so the layers stop being protected the day the harness renames `Edit` or adds `ApplyPatch`/`StrReplace`. Do not let an unknown or non-string tool name take the lenient path. Do not claim anywhere that the knowledge layers are protected "outside the model" without the in-place-only caveat — that sentence was written once and was false. Do not close the hole by blocking `Write` before the sync commands have been moved off the `Write` tool. Do not match on a path prefix or with case sensitivity: macOS ships a case-insensitive filesystem, and `..` must be normalised before segments are compared. Keep the hook fail-open on any parse error — a hook that throws blocks every edit in the repository.
**Ticket:** GL-926

## Pre-existing native-float arithmetic is a review note, not a CRITICAL

**Date:** 2026-08-24
**Decision:** `check-financial.md` section 1 (money must use `Decimal.js`) is enforced as CRITICAL only for arithmetic **this change introduces**. Float arithmetic that already existed and was merely touched, moved or sat next to is reported once as a note. The carve-out is written into `fx-review/SKILL.md` boundary 3, which otherwise forbids downgrading any money finding.
**Alternatives considered:** Enforce section 1 strictly — rejected, see Reason. Drop the Decimal.js rule — rejected, it is a `CLAUDE.md` requirement and float errors in hedging are production incidents. Migrate the model to `Decimal.js` as part of introducing the skill set — rejected as far too large and too risky to bundle.
**Reason:** Of 125 non-test `.ts`/`.tsx` files under `lib/` and `components/`, exactly **one** imports `decimal.js` (`lib/treasury/snapshot.ts`); the core model (`lib/fx-buffer.ts` and the calculators around it) is native float throughout. Enforced strictly, the first review of any `lib/fx-*.ts` change returns a wall of CRITICALs the change did not cause; `fx-implement` hard-stops after three review rounds; and no change to the financial model can land through the documented workflow. A gate that blocks everything gets switched off, and then it protects nothing.
**Anti-patterns:** Do not extend this carve-out beyond pre-existing float. Secrets, the Treasury write boundary, financial data reaching a log, and float arithmetic introduced by the change under review all stay CRITICAL with no exception. Do not migrate the model to `Decimal.js` as a drive-by inside an unrelated change — it needs its own ticket, hand-verification of every affected number, and FX lead sign-off. Do not grep the bare word `Decimal` to measure adoption: it matches the chart-axis props `xDecimals`/`yDecimals` in four components that do not use Decimal.js, and doing so once overstated adoption fourfold inside this very checklist.
**Ticket:** GL-926

## `.env.example` and the README table stay fully in sync; commented is documented, not optional

**Date:** 2026-08-24
**Decision:** The two configuration documents describe the same set of variables and are kept in full sync, checked by `.claude/skills/fx-review/scripts/env-drift.mjs`. An entry in `.env.example` has **three** states, not two: an active `NAME=` declaration, a commented `# NAME=` (documented, optional-looking), and absent. Both of the first two count as documented; requiredness is read from the README `Required` column, never inferred from the comment.
**Alternatives considered:** Treat the README table as a Treasury-only excerpt, which is what it previously claimed to be — rejected, the two then drift silently. Treat a commented entry as undocumented — rejected: it reported 11 false positives on entries that are documented perfectly well, and a tool that cries wolf gets ignored.
**Reason:** The rule was already in `CLAUDE.md` and was being violated at the time this was written: 29 variables in code against 13 active declarations, with `AUTH_URL` — marked required in production — and `TOKEN_ENCRYPTION_KEY_PREVIOUS` present in the README table and absent from `.env.example`. Both documents are now synchronised and the checker reports `IN SYNC`.
**Anti-patterns:** Do not read "commented out" as "optional" — `AUTH_URL` is commented in `.env.example` and still required in production. Do not add a third list of environment variables anywhere; `setup.md` used to carry a `TODO` stub table and now points at the README instead. Do not scan `scripts/` for `process.env` — it holds build tooling (`APPDATA`, `NEXT_FORCE_LOCAL_DIST`) that is not application configuration, and the noise trains people to ignore the report. Do not treat `RESULT: SCAN FAILED`, or a silent exit, as `IN SYNC` — it means the scan never ran. Do not omit runtime-provided names (`NODE_ENV`, `VERCEL_ENV`, `NODE_PATH`) from the README; list them in their own "set by the runtime" section so the list is complete without inviting anyone to set them.
**Ticket:** GL-926
