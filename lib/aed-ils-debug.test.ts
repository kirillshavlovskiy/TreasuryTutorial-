...
describe('AED/ILS Target LP Cash USD', () => {
  ...
  it('AED/ILS expensive-OD: zero payout → Target equals LP+Swap (raw may be negative)', () => {
    ...
    expect(aed.postSwapCash).toBeCloseTo(aed.lp_after_swap_trough!, 4);
    expect(aed.lp_after_swap_trough).toBeCloseTo(aed.lp_peak_cash + aed.swapNear, 4);
  ...
  it('pre-swap H* preserved separately from wired target', () => {
    const aed = model.fcyComputed.find(x => x.ccy === 'AED')!;
    expect(aed.cash_threshold).toBeCloseTo(aed.lp_peak_cash + aed.swapNear, 4);
