/**
 * Layer-by-layer stacking audit — documents Target NP Cash as layers are added.
 */
...
describe('layer-by-layer stacking audit', () => {
  it('CAD PAY −100M: incremental layers (documents numbers)', () => {
    ...
    // No formula layers → swap reverses trough back to 0; Target = NP+Swap = |payout| exactly
    expect(table.none!.target).toBeCloseTo(100, 1);
    expect(table.none!.npSwap).toBeCloseTo(100, 1);
    expect(table.none!.trough).toBeCloseTo(-4.9, 1);

    // Sigma on gross |payout|: cushion H* = σ (survives the payout);
    // Target = NP+Swap = |payout| + σ (funds the payout plus the cushion)
    ...
    expect(table['+sigma']!.npSwap).toBeCloseTo(100 + expectedSigma, 1);
