...
export type DeskStepperLayout = 'stack' | 'inline';
...
const STEP_BTN =
  'inline-flex shrink-0 items-center justify-center rounded border border-slate-600 bg-slate-950 font-semibold text-slate-300 hover:border-slate-500 hover:bg-slate-800 hover:text-white disabled:cursor-not-allowed disabled:opacity-40';
const STEP_BTN_STACK = `${STEP_BTN} h-6 w-6 text-[11px]`;
const STEP_BTN_INLINE = `${STEP_BTN} h-5 w-5 text-[10px]`;
...
export function DeskStepper({
  ...
  /** `inline` = one row, button height — for toolbars next to Hedge path / Cover. */
  layout?: DeskStepperLayout;
}) {
  ...
  const stepBtn = inline ? STEP_BTN_INLINE : STEP_BTN_STACK;
  ...
  const track = (
    ...
    >
      ...
      >
        ...
        <span
          className={`pointer-events-none absolute top-1/2 -translate-x-1/2 -translate-y-1/2 rounded-full border-2 bg-slate-950 shadow-md ${hue.thumb} ${
